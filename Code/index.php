...
<html>
    <head>
        ...
        <script src = "FrontUsuario/Librerias/jquery-3.3.1.min.js"></script>
        <script>
			function Agrega(id_usuario, id_oferta)
			{
                ...
                $.ajax(
                {
                    url : 'FrontUsuario/Ofertas/AgregaOferta.php',
                    type : 'post',
                    dataType : 'text',
                    data : parametros,
                    ...
                    success : function(res)
                    {
                        if(res)
                        {
                            $.ajax(
                            {
                                url : 'FrontUsuario/EstadoCarrito.php',
                                type : 'post',
                                dataType : 'text',
                                data : parametros,
            ...
            function Detalle(id)
			{
				window.location.href = 'FrontUsuario/Ofertas/oferta_detalle.php?id='+ id;
			}
        </script>
    ...
    <body>
		<?php
            ...
            require "FrontUsuario/Funciones/conecta.php";
			$con = conecta();
            ...
            $maxO = mysqli_num_rows($res);

            $numbersList = randomGen(0,$maxO - 1,8);
		...
		<div class="header">
                  ...
                  <div class="col-xl-3 col-lg-3 col-md-3 col-sm-3 col logo_section">
                     <div class="full">
                        <div class="center-desk">
                           <div class="logo">
                              <a href="index.php"><img src="FrontUsuario/Styles/logo.png" alt="#" /></a>
                           </div>
                        </div>
                     </div>
                  </div>
                        ...
                        <nav class="navigation navbar navbar-expand-md navbar-dark ">
                           ...
                           <div class="collapse navbar-collapse" id="navbarsExample04">
                              <ul class="navbar-nav mr-auto">
								 ...
								 <li class="nav-item">
								 <?php  echo "<a class='nav-link' href =\"FrontUsuario/Perfil/usuarios_consultar.php?id=$id_usuario\"'>Perfil</a>";?>
                                 </li>
								 <li class="nav-item">
                                    <a class="nav-link" href="FrontUsuario/Ofertas/ofertas.php?pagina=1&">Ofertas</a>
                                 </li>
								 <li class="nav-item">
                                    <a class="nav-link" href="FrontUsuario/Contacto/contacto_formulario.php">Contacto</a>
                                 </li>
								 <li class="nav-item">
                                    <a class="nav-link" href="FrontUsuario/Carrito/carrito_paso01.php">Postulaciones</a>
								</li>
                              </ul>
                              <div class="sign_btn"><a href="FrontUsuario/CerrarSesion.php">Cerrar Sesión</a></div>
                           </div>
                        </nav>
                     </div>
                  </div>
          ...
          <div class='banner'>
            ...
            font-weight: bold; color:#00aeef; position:absolute;top:40%;left:30%;'>Aquí empieza tu futuro</div>
            ...
            top:60%;left:46%;' href="#ofertas">Ver Ofertas</a>
        </div>
        <?php
            $sql1 = "SELECT * FROM ofertas_solicitadas WHERE id_usuario = $id_usuario AND estado = 0 ";
        ...
        <?php
            ...
            //echo"<img class='normal' src='FrontAdministrador/Banners/Archivos/".$archivo."'";
            ...
            foreach($res as $row)
            {
                if($maxO > 7)
                {
                    if($i == $numbersList[7])
                    {
                        ...
                        if($eliminado == 0)
                        {
                            echo "<button style='z-index: 4;top: 190%;left:33%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick='return Agrega($id_usuario, $id1)'>Postularse</button>";
            ...
            foreach($res as $row)
            {
                if($maxO > 0)
                {
                    if($i == $numbersList[0])
                    {
                        ...
                        if($eliminado == 0)
                        {
                            echo "<button style='z-index: 4;top: 190%;left:33%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick='return Agrega($id_usuario, $id2)'>Postularse</button>";
            ...
            foreach($res as $row)
            {
                if($maxO > 1)
                {
                    if($i == $numbersList[1])
                    {
                        ...
                        if($eliminado == 0)
                        {
                            echo"<button style='z-index: 4;top: 190%;right:8%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick = 'return Agrega($id_usuario, $id2)'>Postularse</button>";
            ...
            foreach($res as $row)
            {
                if($maxO > 2)
                {
                    if($i == $numbersList[2])
                    {
                        ...
                        if($eliminado == 0)
                        {
                            echo "<button style='z-index: 4;top: 270%;left:33%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick='return Agrega($id_usuario, $id3)'>Postularse</button>";
            ...
            foreach($res as $row)
            {
                if($maxO > 7)
                {
                    if($i == $numbersList[3])
                    {
                        ...
                        if($eliminado == 0)
                        {
                            echo"<button style='z-index: 4;top: 270%;right:8%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick = 'return Agrega($id_usuario, $id2)'>Postularse</button>";
            ...
            foreach($res as $row)
            {
                if($maxO > 4)
                {
                    if($i == $numbersList[4])
                    {
                        ...
                        echo '<div style="font-size:20px;z-index: 4; color: #000; font-family: \'Inter UI\', sans-serif; font-weight: bold; position: absolute; top: 265%; left:62%;" href="javascript:void(0);" onclick="return Detalle('.$id6.')">'.$nombre.'</div>';
                        echo'<div style="font-size:20px;z-index: 4; color: #000; font-family: \'Inter UI\', sans-serif; position: absolute; top: 270%; left:62%;",id = "'.$id6.'"> '."$ ".$sueldo6.' </div>';

                        if($eliminado == 0)
                        {
                            echo "<button style='z-index: 4;top: 270%;right:8%;background-color: #00aeef; width: 8%; height: 5%; color: #000; font-weight: bold; font-family: \"Inter UI\", sans-serif; border-radius: 30px; position: absolute; display: flex; justify-content: center; align-items: center;' onclick='return Agrega($id_usuario, $id3)'>Postularse</button>";
